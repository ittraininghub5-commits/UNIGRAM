...
import { Share2, Edit3, Award, Quote } from 'lucide-react';
...
export default function ProfilePage({ currentProfile }: ProfilePageProps) {
  ...
  // Fix 1 — stable ref for profile so realtime callbacks don't go stale
  const profileRef = useRef<Profile | null>(null);
  profileRef.current = profile;

  // Fix 2 — derive targetId once; don't put whole currentProfile in deps
  const targetId = id || currentProfile?.id;

  const fetchProfileStats = useCallback(async (targetProfile: Profile) => {
    try {
  ...
  const fetchProfileCertificates = useCallback(async (targetProfile: Profile) => {
    try {
      const isMentor = isMentorRole(targetProfile.role);
  ...
  const fetchProfileRecommendations = useCallback(async (targetProfile: Profile) => {
    try {
      const isMentor = isMentorRole(targetProfile.role);
      const { data, error } = await supabase
  ...
  // Fix 2 — depend only on targetId (a string), not the whole currentProfile object
  }, [targetId, fetchProfileStats, fetchProfileCertificates, fetchProfileRecommendations]);

  useEffect(() => {
    fetchProfile();
  }, [fetchProfile]);

  // Fix 3 — narrow realtime subscriptions; use profileRef to avoid stale closures
  useEffect(() => {
    if (!targetId || !profile) return;

    const channel = supabase
      .channel(`profile-live-${targetId}`)
      // Only this profile's own row — already filtered correctly
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles', filter: `id=eq.${targetId}` }, () => {
        fetchProfile();
      })
      // Fix 3a — filter follows to only rows involving this profile
      .on('postgres_changes', { event: '*', schema: 'public', table: 'follows', filter: `following_id=eq.${targetId}` }, () => {
        const p = profileRef.current;
        if (p) fetchProfileStats(p);
      })
      // Fix 3b — filter courses/enrollments to this profile
      .on('postgres_changes', { event: '*', schema: 'public', table: 'courses', filter: `mentor_id=eq.${targetId}` }, () => {
        const p = profileRef.current;
        if (p) fetchProfileStats(p);
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'enrollments', filter: `student_id=eq.${targetId}` }, () => {
        const p = profileRef.current;
        if (p) fetchProfileStats(p);
      })
      // Fix 3c — certificates filtered; use ref to avoid stale closure on profile
      .on('postgres_changes', { event: '*', schema: 'public', table: 'certificates', filter: `student_id=eq.${targetId}` }, () => {
        const p = profileRef.current;
        if (p) { fetchProfileStats(p); fetchProfileCertificates(p); }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'recommendations', filter: `mentor_id=eq.${targetId}` }, () => {
        const p = profileRef.current;
        if (p) fetchProfileRecommendations(p);
      })
      .subscribe();
  ...
  useEffect(() => {
    const fetchFollowState = async () => {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) { console.error('Error checking follow state:', error); return; }
      setIsFollowing(!!data);
    };
    fetchFollowState();
  ...
  const handleToggleFollow = useCallback(async () => {
    if (!currentProfile?.id || !profile || profile.id === currentProfile.id) return;
    try {
      if (isFollowing) {
  ...
  }, [currentProfile, profile, isFollowing, navigate]);

  const handleSaveProfile = useCallback(async () => {
    if (!profile?.id || profile.id !== currentProfile?.id) return;
  ...
  }, [profile, currentProfile?.id, draftName, draftInstitution, draftAvatarUrl]);
  ...
  // Fix 4 — memoize derived boolean so it doesn't recompute every render
  const isOwnProfile = useMemo(() => profile?.id === currentProfile?.id, [profile?.id, currentProfile?.id]);
          ...
          <div className="px-5 sm:px-8 pt-5 pb-8">
            <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
              ...
              <div className="flex flex-wrap justify-end gap-2 ml-auto">
                <button onClick={handleShareProfile} className="p-2.5 rounded-xl border border-white/5 hover:bg-white/5 transition-all shrink-0">
...
const ProfileStat = memo(function ProfileStat({ value, label }: { value: string; label: string }) {
  return (
...
const CertCard = memo(function CertCard({ title, mentor, date, icon }: { title: string; mentor: string; date: string; icon: string }) {
  return (
