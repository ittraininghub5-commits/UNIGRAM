...
const CARD_VIEWPORT = { once: true }; // ✅ only animates once — not on every re-render
...
// ─── Subcomponent: SidebarMentorItem ─────────────────────────────────────────
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-text-secondary truncate max-w-[120px]">
            {mentor.institution || 'Expert Mentor'}
          </p>
        </div>
      </div>
      <button
...
// ─── Subcomponent: EnrolledCourseCard ────────────────────────────────────────
...
const PostCard = memo(function PostCard({
  video, isFollowingMentor, onToggleFollow, viewerRole,
}: {
  video: Video;
  ...
  useEffect(() => {
    if (video.id) getAIInsightForVideo(video.id).then(setInsight);
  }, [video.id]);

  // ✅ useCallback for all handlers
  const handleCardClick = useCallback(() => {
    if (studentCardClickable) navigate(`/course/${video.course_id}`);
  }, [studentCardClickable, navigate, video.course_id]);

  const handleFollowClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (video.mentor?.id) void onToggleFollow(video.mentor.id);
  }, [video.mentor?.id, onToggleFollow]);
 ...
 toast.error('Mentor ID not available for messaging.');
    }
  }, [navigate, video.mentor?.id]);
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-accent-teal/20 flex items-center justify-center text-xs font-bold text-accent-teal">
            {video.mentor?.avatar_url ? (
              <img src={video.mentor.avatar_url} alt={video.mentor.full_name} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy"
...
// ─── Subcomponent: CourseAnnouncementCard ────────────────────────────────────
const CourseAnnouncementCard = memo(function CourseAnnouncementCard({
  ...
  const handleCardClick = useCallback(() => {
    if (studentCardClickable) navigate(`/course/${course.id}`);
  }, [studentCardClickable, navigate, course.id]);
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full overflow-hidden bg-accent-teal/20 flex items-center justify-center text-xs font-bold text-accent-teal">
              {course.mentor?.avatar_url ? (
                <img src={course.mentor.avatar_url} alt={course.mentor.full_name} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy"
                  onError={(e) => { e.currentTarget.style.display = 'none'; const f = e.currentTarget.nextElementSibling as HTMLElement | null; if (f) f.style.display = 'flex'; }} />
              ) : null}
              <span style={{ display: course.mentor?.avatar_url ? 'none' : 'flex' }}>
...
export default function FeedPage({ profile }: FeedPageProps) {
  ...
  // ✅ Optimized fetch functions — select only needed fields
  ...
  const fetchVideos = useCallback(async () => {
    ...
    // Note: Removed the hard .eq('course.status', 'live') because it forces an INNER JOIN,
    // which hides all general posts/glimpses that don't have an associated course.
    // If strict filtering is needed, it should be done via a left-join compatible filter or logic.

    if (activeTab === 'Following') {
      if (followedMentorIds.size === 0) { setVideos([]); return; }
      query = query.in('mentor_id', Array.from(followedMentorIds));
    }
    query = activeTab === 'Trending'
      ...
      if (activeTab === 'For You') {
        ...
        processedVideos.sort((a, b) => {
          const score = (video: Video) => {
            ...
            if (followedMentorIds.has(video.mentor_id)) value += 100;
  ...
  }, [activeTab, followedMentorIds, profile?.institution]);
  ...
  const fetchCourseFeed = useCallback(async () => {
    let query = supabase
      ...
      .eq('status', 'live');
    if (activeTab === 'Following') {
      if (followedMentorIds.size === 0) { setCourseFeed([]); return; }
      query = query.in('mentor_id', Array.from(followedMentorIds));
    }
  ...
  const fetchMentorOwnedCourses = useCallback(async () => {
    ...
    const { data, error } = await supabase // Added mentor relation
      .from('courses')
  ...
  // ✅ Effects — each with minimal deps
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        else { await Promise.all([fetchVideos(), fetchCourseFeed()]); }
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'videos' }, () => {
        if (activeTab !== 'My Courses') void fetchVideos();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'courses' }, () => {
        if (activeTab !== 'My Courses') void Promise.all([fetchVideos(), fetchCourseFeed()]);
        void fetchMentorOwnedCourses();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, () => void fetchMentors())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'follows' }, () => {
        void fetchFollows();
        if (activeTab === 'Following') void Promise.all([fetchVideos(), fetchCourseFeed()]);
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'enrollments' }, () => void fetchEnrollments())
      .subscribe();
  ...
  }, [profile?.id, followedMentorIds, navigate]);
  ...
  }, [profile?.id, profile?.role, glimpseTitle, glimpseDescription, glimpseCourseId, fetchVideos, fetchCourseFeed]);
        ...
        <div className="space-y-8 min-w-0 lg:order-2">
              ...
              <button key={mentor.id} className="flex-shrink-0 group space-y-2 text-center w-20"
                ...
                <div className="w-16 h-16 rounded-full p-0.5 bg-gradient-to-tr from-accent-teal to-accent-purple group-hover:scale-105 transition-transform">
                  <div className="w-full h-full rounded-full overflow-hidden bg-bg-card border-2 border-bg-base flex items-center justify-center text-xl font-bold text-accent-teal">
                    {mentor.avatar_url ? (
                      <img src={mentor.avatar_url} alt={mentor.full_name} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy"
                        onError={(e) => { e.currentTarget.style.display = 'none'; const f = e.currentTarget.nextElementSibling as HTMLElement | null; if (f) f.style.display = 'flex'; }} />
                    ) : null}
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              ? feedItems.map((item) =>
                  item.type === 'video'
                    ? <PostCard key={item.id} video={item.video} isFollowingMentor={!!item.video.mentor?.id && followedMentorIds.has(item.video.mentor.id)} onToggleFollow={handleToggleFollow} viewerRole={profile?.role} />
                    : <CourseAnnouncementCard key={item.id} course={item.course} isFollowingMentor={!!item.course.mentor?.id && followedMentorIds.has(item.course.mentor.id)} onToggleFollow={handleToggleFollow} viewerRole={profile?.role} />
                )
              : (
                <div className="editorial-card rounded-3xl p-20 text-center space-y-4">
                  <div className="w-20 h-20 bg-bg-elevated rounded-full flex items-center justify-center mx-auto text-3xl">🎥</div>
          ...
          <div className="editorial-card lift-on-hover rounded-[30px] p-6 text-center space-y-4">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#43c0ff] to-[#2b7fff] p-[2px] mx-auto shadow-[0_12px_28px_rgba(67,192,255,0.22)]">
              <div className="w-full h-full rounded-full bg-bg-card overflow-hidden flex items-center justify-center text-[2rem] font-bold text-[#43c0ff]">
                {profile?.avatar_url ? (
                  <img src={profile.avatar_url} alt={profile.full_name} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy"
                    onError={(e) => { e.currentTarget.style.display = 'none'; const f = e.currentTarget.nextElementSibling as HTMLElement | null; if (f) f.style.display = 'flex'; }} />
                ) : null}
